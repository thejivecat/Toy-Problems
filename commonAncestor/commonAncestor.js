  ...
  if (this.isDescendant(node1) && this.isDescendant(node2)) {
    for (let i = 0; i < this.children.length; i++) {
      if (this.children[i].isDescendant(node1) && this.children[i].isDescendant(node2)) {
        ...
        this.children[i].getClosestCommonAncestor(node1, node2);
      } else {
        return ancestor;
      }
    }
...
Tree.prototype.getAncestorPath = function(node) {
  ...
  if (this === node) {
    return [this];
  } else if (!this.isDescendant(node)) {
    return null;
  }
  const recurse = (nodes) => {
    if (nodes.isDescendant(node)) {
      for (let i = 0; i < nodes.children.length; i++) {
        if (nodes.children[i].isDescendant(node)) {
          path.push(nodes.children[i]);
          recurse(nodes.children[i]);
        } else {
          path.push(node);
        }
      }
